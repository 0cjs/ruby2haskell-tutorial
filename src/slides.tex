...
\begin{itemize}
    \item But hang on! In the last slide we used a lambda for the
        paramter to \T{filter} in Haskell, but not in Ruby:
        \begin{rlisting}
            (0..7).select { |x| x.even? }
...
\begin{itemize}
    \item In the comparisions of Ruby and Haskell code in previous slides,
    ...
    \item \T{inejct} is less commonly in Ruby, but its Haskell equivalant,
...
\begin{itemize}
    ...
    \item We can have multiple definitions of a function so long as their
        ...
        pattern is used.
    \note[item]{ The observant among you will note that chosing the first
...
\note[enumerate]{
    ...
    \item GHC can't automtaically infer the type of that last
...
\begin{itemize}
    ...
    \note[item]{ GHC, with the \T{-XUnicodeSyntax} option, also allows
        us to use Unicode characters for synatx if we want to look really
        nice, e.g.,\\
